...
const { getInquiries } = require('../googleSheets');
...
router.get('/all', requireManager, async (req, res) => {
  try {
    ...
    for (const [agentName, sheetId] of Object.entries(AGENT_SHEETS)) {
      ...
      try {
        const rows = await getInquiries(sheetId, agentName);
        const statuses = db.prepare('SELECT * FROM inquiry_statuses WHERE agent = ?').all(agentName);
...
router.get('/:agentName/statuses', requireAuth, (req, res) => {
  ...
  const rows = db.prepare('SELECT * FROM inquiry_statuses WHERE agent = ?').all(agentName);
  res.json(rows);
});
...
router.post('/:agentName/status', requireAuth, (req, res) => {
  ...
  db.prepare(`
    INSERT INTO inquiry_statuses (agent, sheet_row_key, status, comment)
    VALUES (?, ?, ?, ?)
